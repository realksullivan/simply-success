...
/
          ...
          <div className="bg-[#0D1929] border border-[#1E3550] rounded-xl p-5">
            ...
            {focusTask ? (
              <div className="flex items-center gap-3 bg-[#0F2A50] rounded-lg px-4 py-3 border-l-4 border-[#C8922A]">
                <Checkbox checked={focusTask.is_done} onChange={() => toggleTask(focusTask.id, focusTask.is_done)} />
                <span className={`text-[#F4F0E8] text-sm font-medium ${focusTask.is_done ? 'line-through text-[#3A5070]' : ''}`}>
                  {focusTask.title}
                </span>
              </div>
            ) : (
          ...
          <div className="bg-[#0D1929] border border-[#1E3550] rounded-xl p-5">
            ...
            {otherTasks.map((t, i) => (
              <div key={t.id} className={`flex items-center gap-3 py-2 ${i < otherTasks.length - 1 ? 'border-b border-[#1E3550]' : ''}`}>
                <Checkbox checked={t.is_done} onChange={() => toggleTask(t.id, t.is_done)} />
                <span className={`text-sm ${t.is_done ? 'line-through text-[#3A5070]' : 'text-[#F4F0E8]'}`}>{t.title}</span>
              </div>
            ))}
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="bg-[#0D1929] border border-[#1E3550] rounded-xl p-5">
            ...
            {habits.length === 0 && (
              <div className="text-[#3A5070] text-sm italic py-2">No habits set up yet</div>
            )}
            {habits.map((h, i) => {
              const log = habitLogs.find(l => l.habit_id === h.id)
              const done = log?.is_done || false
              return (
                <div key={h.id} className={`flex items-center gap-3 py-2 ${i < habits.length - 1 ? 'border-b border-[#1E3550]' : ''}`}>
                  <Checkbox checked={done} onChange={() => toggleHabit(h.id, done)} color="green" />
                  <span className={`text-sm ${done ? 'line-through text-[#3A5070]' : 'text-[#F4F0E8]'}`}>{h.title}</span>
                </div>
              )
            })}
          </div>
